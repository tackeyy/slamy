...
describe("local session Web API client", () => {
  ...
  it("allows conversations.inviteShared through the local session", async () => {
    const request = vi.fn().mockResolvedValue({ ok: true });
    const connection = {
      version: 1 as const,
      teamId: parseTeamId("T0BJ9SG2M0R"),
    ...
    await client.conversations.inviteShared({ channel: "C0123ABC", emails: ["advisor@example.com"] });

    expect(request).toHaveBeenCalledWith(connection, "conversations.inviteShared", {
      channel: "C0123ABC", emails: ["advisor@example.com"],
    });
  });
